...
(function () {
  ...
  const html = `
    ...
    <div id="cs-chat-panel" role="dialog" aria-label="Chat de atendimento" aria-modal="false">
      ...
      <form id="cs-chat-form" autocomplete="off">
        <input id="cs-chat-input" type="text" placeholder="Tire sua dúvida…" maxlength="400" aria-label="Sua mensagem" />
  ...
  document.querySelector('.header')?.addEventListener('click', (e) => {
    if (!opened) return;
    if (e.target.closest('.nav-chat-btn, .cs-close')) return;
    closeChat();
  });
  ...
  // Teclado no handle (role="button"): Enter/Espaço expande/recolhe
