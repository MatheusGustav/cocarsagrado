...
-- Rodar SÓ na fase de limpeza, FORA do horário de atendimento, depois
-- que o cache do JS antigo dos clientes já tiver expirado (dias).
--
-- Ao aplicar, COORDENAR junto:
--   1. mover este arquivo para supabase/migrations/<timestamp>_... e db push
--   2. no front (js/agendamento-system.js), remover o p_aceitou_desconto_10
--      da chamada supabase.rpc('criar_pedido', ...) — volta p/ 7 args
--   3. atualizar supabase/sql/setup.sql p/ refletir o novo estado
--   4. remover este aviso e o TODO no front
-- Motivo de esperar: a RPC muda de 8→7 args; clientes com JS em cache
-- ainda mandam 8 args. Trocar antes do cache expirar quebra o checkout.
-- ============================================================
