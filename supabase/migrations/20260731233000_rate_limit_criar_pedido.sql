-- Rate limit de criação de pedido (decisão 2026-07-31, backlog item 1).
-- Barra na porta, sem robô apagando depois — pendente é estado legítimo
-- do "pagar de outra forma" (cliente combinando com o admin no zap).
--   (a) máx. 2 pedidos pendentes em aberto por WhatsApp;
--   (b) máx. 3 pedidos criados em 24h por WhatsApp, contados num log
--       próprio que sobrevive à exclusão do pedido — recriar logo depois
--       da limpeza manual do admin não zera o contador.

-- Log de criações por WhatsApp (só dígitos). Sem grants: só a
-- criar_pedido (SECURITY DEFINER) toca aqui; a própria função apaga
-- linhas com mais de 48h (autolimpeza, sem cron).
...
BEGIN
  ...
  -- Rate limit por WhatsApp: barra na porta (o pedido nem nasce), destrava
  -- sozinho ao pagar ou quando o admin exclui pendente. Comparação por
  -- dígitos casa com o índice idx_pedidos_whatsapp.
