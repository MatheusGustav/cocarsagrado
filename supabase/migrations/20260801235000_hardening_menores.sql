-- ============================================================
-- Endurecimento (itens menores do review de segurança 31/07):
-- 1) validar_cupom: rate limit global de falhas (30/h) — anon não tem
--    identidade, então o teto é coletivo; chute de código vira inútil.
-- 2) criar_pedido: rejeita data de agendamento no passado (fuso SP).
-- 3) configuracoes: revoga escrita de anon (grant morto de 20260514050516;
--    a RLS já bloqueava, o grant estava pendurado à toa).
-- 4) set_updated_at: search_path fixo (advisor).
-- ============================================================

-- Tentativas FALHAS de cupom (autolimpeza 2h dentro da validar_cupom).
