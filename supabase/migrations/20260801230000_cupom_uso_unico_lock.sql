-- Item 7 do backlog de segurança (review 31/07): cupom de uso único tinha
-- race — dois pedidos simultâneos passavam no EXISTS antes de qualquer um
-- commitar. Fix: pg_advisory_xact_lock por código do cupom serializa a
-- checagem. Mantém a janela de 24h do pendente (que índice único não
-- expressa) e não afeta cupom multi-uso (lock só quando uso_unico).
